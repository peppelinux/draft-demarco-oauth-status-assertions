...
If the Digital Credential is valid, the Credential Issuer MUST return an HTTP status code of 201 (Created), with the content type set to `application/json`. The response MUST include a JSON object with a member named `status_assertion`, which contains an array of strings. Each string in the array represents a Status Assertion, as illustrated in the following non-normative example:
