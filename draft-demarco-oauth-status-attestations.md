...
| **kid** | Unique identifier of the `JWK` or `Cose_Key` used for the signature of the Status Attestation Request, it MUST match the one contained in the Credential. Otherwise, the `kid` is OPTIONAL when `x5c` is present. | {{RFC7515}} |
